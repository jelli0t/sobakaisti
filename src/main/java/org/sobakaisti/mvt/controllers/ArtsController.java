...
public class ArtsController {
	...
	@GetMapping(value="/{category}/by/{author}")
	...
	public String showArticleBySlug(
...
//			model.addAttribute("authors", authors);
			...
			List<Comment> postComments = commentService.findPostComments(fullArticle.getId(), PostOrigin.ARTICLE, 0, 3);
			if(postComments != null && postComments.size() > 0) {
				model.addAttribute("postComments", postComments);
				model.addAttribute(CommentService.COMMENTS_COUNT_MODEL_ATTRIBUTE_NAME, 
						commentService.countPostComments(fullArticle.getId(), PostOrigin.ARTICLE));
			}
...
//		}
	...
	private void populateCommentsCountPerPostIndicator(Model model) {
		Map<Integer, Integer> postToCommentsCountMap = commentService.getPostToCommentsCountMap(PostOrigin.ARTICLE);
